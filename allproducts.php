...
<br><h3><?php echo $_GET['auctiontype'];?></h3><hr>
		<?php
		$sqlcategory = "select * from category WHERE status='Active' AND category_id='$_GET[category_id]'";
		$qsqlcategory = mysqli_query($con,$sqlcategory);
		while($rscategory = mysqli_fetch_array($qsqlcategory))
		{
			$sqlproduct = "select * from product WHERE status='Active' AND category_id='$_GET[category_id]'  AND start_date_time<='$dttim'  AND product.customer_id!='0' ";
